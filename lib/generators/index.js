...
const mock = (schema, example)  => {
    ...
    if (schema) {
        ...
        if (example) {
            ...
            if (generator) {
                mock = generator.call(null, schema, null);
            }
        }
    }
    return mock;
...
const compositionMock = ({ allOf }, useExample) => {
    
    let compositionSchema = {}
    compositionSchema.properties = {}

    allOf.forEach(part => {
        Object.keys(part.properties).forEach( property => {
            compositionSchema.properties[property] = part.properties[property]
        })
    })
    console.log(compositionSchema)
    let mockObj = objectMock(compositionSchema, useExample)
    return mockObj
}
...
const integerMock = schema => {
    ...
    if (Number.isInteger(schema.multipleOf) && schema.multipleOf > 0) {
        ...
        opts.min = (Number.isInteger(opts.min)) ? opts.min : 1;
        //Use the max/muplilier as the new max value
        //Use a default - min + 10 - if max value is not properly set.
        opts.max = (Number.isInteger(opts.max)) ? opts.max : (opts.min + 10);
        intmock = Chance.integer(opts);
    }
    return intmock;
};
