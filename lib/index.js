...
const mockResponses = (opsObj, options) => {
    ...
    if (responses) {
        ...
        if (response) {
            //Found the response
            mockResp = mockResponse(response, options);
        } else {
            mockResp = mockResp || {};
...
const mockResponse = (response, options) => {
    ...
    if (content) {
        mockResp = mockContents(content, options)
    }
    return mockResp;
};

const mockContents = (schemaObj, options) => {
    let mockedContent;
    if (schemaObj) {
        let content = schemaObj['application/json'];
        if (content) {
            //Found the response
            mockedContent = mockContent(content, options);
        } else {
            mockedContent = {};
        }
    }
...
const mockContent = (contentObj, options) => {
    let mockResp;
    let schema = contentObj.schema;
    if (schema) {
        let example = options.useExamples ? (contentObj.example || contentObj.examples) : null
        mockResp = Generators.mock(schema, example);
    }
    return mockResp;
}
